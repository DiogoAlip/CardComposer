...
export const GameRoundProvider = ({
  children,
}: {
  ...
  const setOnePlayersName = (name: string, player: 1 | 2) => {
    if (player === 1) {
      setPlayersName({ ...playersName, P1Name: name });
    } else {
      setPlayersName({ ...playersName, P2Name: name });
    }
  };

  const setBothPlayersNames = ({
    player1,
    player2,
  }: {
    player1: string;
