...
// Mocking getRankToValue is not strictly necessary as we have tests for it,
// but it makes these tests purely about the match logic.
// However, since it's a small pure function, using the real one is often fine.
// I'll use the real one to ensure integration works as expected.
...
describe("getMatch.ts", () => {
    describe("getMatchCards", () => {
        it("should return Player1 as winner when rank is higher", () => {
            ...
            expect(result.score).toBe(1); // 13 - 12
        });
        ...
        it("should return Player2 as winner when rank is higher", () => {
            ...
            const result = getMatchCards(p1, p2);
            expect(result.winner.name).toBe("P1"); // 2 vs 1
    ...
    describe("evaluateMatchup", () => {
        const createEmptyRows = () => ({
            FrontRow: Array(4).fill(null).map(() => createCard()),
            BackRow: Array(4).fill(null).map(() => createCard({ isIt: false }))
        });
