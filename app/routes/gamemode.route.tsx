  ...
  const gameModes = [
    {
      ...
      title: "Solo",
      description:
        "Desafía al Bot y demuestra tu dominio de la programación funcional",
      icon: Bot,
      avaible: true,
      children: (
        <div className="flex flex-row-reverse gap-2">
          <Button
            className="w-28 text-white border-primary bg-black/50 hover:bg-black/30"
      ...
      children: (
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-row text-center py-2 px-10 bg-black/50 rounded-lg">
            ...
            <button
              className="relative hover:text-white/70"
              onClick={() => navigator.clipboard.writeText("21K2-1A3J")}
            >
              <span>
                <ClipboardIcon className="absolute bottom-1 top-1 left-4 h-4 w-4" />
              </span>
            </button>
          </div>
          <Link
          ...
          >
            <p className="text-white text-[16px] text-font-bold">
              Entrar a la Sala
            </p>
          ...
          <div className="container mx-auto max-w-6xl">
            <div className="mb-16 text-center">
              ...
              <p className="text-white/70 text-lg max-w-2xl mx-auto">
                Selecciona cómo deseas jugar y comienza tu aventura en
                CardComposer
              </p>
            </div>

            <div
              className={`${gameModeSelected.length ? "flex flex-col justify-center items-center" : "grid md:grid-cols-3 gap-6 mb-12"}`}
            >
              {gameModes.map((mode) => {
