...
describe("cards.store.ts", () => {
    beforeEach(() => {
        ...
        const testCards = Array(16).fill(null).map((_, i) => createCard({ rank: (i + 1).toString() as any }));
    ...
    it("ShuffleCards should update all rows and initial states", () => {
        ...
        // Check that something changed (statistically almost certain)
        expect(newState.CardsFromPlayer1).not.toEqual(initialState.CardsFromPlayer1);
    ...
    it("SetCardsInOnePlayer should update FrontRow only if BackRow not provided", () => {
        const newFront = [createCard({ rank: "K" }), createCard({ rank: "K" }), createCard({ rank: "K" }), createCard({ rank: "K" })];
    ...
    it("SetCardsInBothPlayers should update both players", () => {
        const p1Cards = Array(8).fill(null).map(() => createCard({ rank: "2" }));
        const p2Cards = Array(8).fill(null).map(() => createCard({ rank: "3" }));
