...
import { flipFrontDeckCards, extractByFlip, extractByColor, resetCode } from "./cards.thunk";
...
describe("cards.thunk.ts", () => {
    beforeEach(() => {
        ...
        const testCards = Array(8).fill(null).map((_, i) => createCard({ rank: (i + 1).toString() as any }));
    ...
    describe("flipFrontDeckCards", () => {
        ...
        it("should set all cards to faceDown when flip is false", () => {
            ...
            useCardsStore.setState({
                CardsFromPlayer1: {
                    FrontRow: useCardsStore.getState().CardsFromPlayer1.FrontRow.map(c => ({ ...c, isFaceUp: true })),
                    BackRow: []
                }
            });
        ...
        it("should alternate faceUp state when alternate is true", () => {
            const initialFrontRow = useCardsStore.getState().CardsFromPlayer1.FrontRow;
            // Mix initial states
            const mixedFrontRow = initialFrontRow.map((c, i) => ({ ...c, isFaceUp: i % 2 === 0 }));
            useCardsStore.setState({ CardsFromPlayer1: { FrontRow: mixedFrontRow, BackRow: [] } });
    ...
    describe("extractByColor", () => {
        it("should set isIt based on card color", () => {
            ...
            useCardsStore.setState({ CardsFromPlayer1: { FrontRow: coloredFrontRow, BackRow: [] } });
    ...
    describe("resetCode", () => {
        it("should call SetInitialState for player 2", () => {
            ...
            const modifiedFrontRow = currentState.FrontRow.map(c => ({ ...c, rank: "Q" as any }));
            useCardsStore.setState({ CardsFromPlayer2: { ...currentState, FrontRow: modifiedFrontRow } });

            resetCode();

            const resetState = useCardsStore.getState().CardsFromPlayer2;
            expect(resetState.FrontRow[0].rank).not.toBe("Q");
            expect(resetState.FrontRow[0].rank).toBe("1"); // Initial rank from setup
        });
    });
});
